...
<jsp:useBean id="ab" class="jspbook.pr43.AddrBean" scope="application" />
<html>
<head>
...
<c:if test="${am.getAddrList().size() == 1}">
	<c:set var="allpage" value="1" />
</c:if>
<c:if test="${am.getAddrList().size() != 1}">	
	<c:set var="allpage" value="${am.getAddrList().size() + 1}" />
...
<c:forEach var="i" begin="1" end="${allpage}">
	<c:if test="${i != currentPage}">
		<a href="addr_list_JSTL.jsp?sindex=${i}">${i}</a>&nbsp;
	</c:if>
	<c:if test="${i == currentPage}">
		${i}&nbsp;
	</c:if>
